...
public class DangerMapAcrivity extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		GlobalConstant.showMessage(DangerMapAcrivity.this,"Press long on a place in the map which is your pickup location");
	...
	public class LoadMapBack extends AsyncTask<String, String, String> {
		...
		protected void onPreExecute() {
			...
			gps = new GPSTracker(DangerMapAcrivity.this);
		}
		...
		protected void onPostExecute(String file_url) {
			...
			runOnUiThread(new Runnable() {
				public void run() {
					...
					GetDangerData dangerMap = new GetDangerData();
					dangerMap.execute();
	...
	private class GetDangerData extends AsyncTask<String, Void, String>{
		...
		private void loadDetails() {
			...
			params.add(new BasicNameValuePair("radious", "100000"));
			params.add(new BasicNameValuePair("lat", String.valueOf(sourceLat)));
			params.add(new BasicNameValuePair("lon", String.valueOf(sourceLon)));
			
			jParser = new JSONParser(DangerMapAcrivity.this);
			json = jParser.makeHttpRequest("http://www.omleteit.com/apps/shadow/GetDangerMap.php", "GET",
					params);
					...
					try {
			            ...
			            if (status.equals(TAG_SUCCESS)) {
			                ...
			                for (int i = 0; i < packs.length(); i++) {
			                    ...
			                    createMarker(new LatLng(nearItem.getDouble("LAT"), nearItem.getDouble("LON")), "Dangerous area", 2);
			                }
			            }
			            else{
			            	isSuccess = false;
